...
class FanficViewSet(ModelViewSet):
    ...
    def comment(self, request, pk):
        ...
        if request.method == 'POST':
            serializer.is_valid(raise_exception=True)
            serializer.save(fanfic=fanfic, owner=self.request.user, text=self.request.data.get('text', None), date_created=clock(), owner_username=self.request.user.username)
            return response.Response(serializer.data, status=201)
        if request.method == 'DELETE':
            delete_id = self.request.data.get('id', None)
